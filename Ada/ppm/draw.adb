...
procedure draw is
  ...
  for Pixel_Array_Type'Component_Size use 24;

  Pixel_Array : Pixel_Array_Type;

  Total_Bytes : constant Ada.Streams.Stream_Element_Offset :=
    Ada.Streams.Stream_Element_Offset(Pixel_Array'Size / 8);
  ...
  Header : constant String :=
    "P6" & ASCII.LF
    & Ada.Strings.Fixed.Trim(Screen_Width'Image, Ada.Strings.Left) & " " & Ada.Strings.Fixed.Trim(Screen_Height'Image, Ada.Strings.Left) & ASCII.LF
    & "255" & ASCII.LF;
...
begin
  ...
  -- for j in Pixel_Array'Range loop
  --   if j mod 50 /= 0 then
  --     Pixel_Array(j) := (
  --       R => 255,
  --       G => 0,
  ...
  Ada.Streams.Stream_IO.Create(File, Ada.Streams.Stream_IO.Out_File, "out.ppm");
