...
procedure Hashmap_Example is
  ...
  procedure Print_Hash_Map(Position : Attendance_Tracker.Cursor) is
    Person : Attendee;
  begin
    Person := Attendance_Tracker.Element(Position);
    Ada.Text_IO.Put_Line(
      "The key: "
      & Ada.Strings.Unbounded.To_String(Attendance_Tracker.Key(Position))
      & ". The data item: Invited = " & Boolean'Image(Person.Attending)
      & ", Vegetarian = " & Boolean'Image(Person.Vegetarian)
      & ", Children = " & Natural'Image(Person.Children)
    );
  end Print_Hash_Map;
...
begin
  ...
  loop
    ...
    then
      ...
      declare
      begin
        Ada.Integer_Text_IO.Get(Children);
        Ada.Text_IO.New_Line;
      exception
        when Ada.Text_IO.Data_Error =>
          Ada.Text_IO.Put_Line("ERROR: The entered value is not an integer, please try again!");
